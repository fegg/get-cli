...
module.exports = function (done) {
  ...
  if (minNodeVersion > currentNodeVersion) {
    return console.log(chalk.red(
      '  You must upgrade node to >=' + minNodeVersion + '.x to use get-cli'
    ))
  }

  request({
    url: 'https://registry.npmjs.org/get-cli',
    timeout: 1000
    ...
    if (!err && res.statusCode === 200) {
      ...
      if (semver.lt(localVersion, latestVersion)) {
        console.log(chalk.yellow('  A newer version of get-cli is available.'))
        console.log()
